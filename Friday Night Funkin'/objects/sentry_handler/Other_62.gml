...
if !is_undefined(requestUuid) {
	...
	if asyncHttpStatus= 200 { //success
		...
		if ds_map_find_value(options,"backup_autoclean") {
	...
	}else if asyncStatus= 0 || !is_undefined(asyncHttpStatus) { //completed, but returned non-200 status
		var resText= "";
		if !is_undefined(asyncHttpStatus) {
			resText+= string_format(asyncHttpStatus,0,0)+" - ";
		}
		if !is_undefined(asyncResult) {
			resText+= asyncResult;
		}
