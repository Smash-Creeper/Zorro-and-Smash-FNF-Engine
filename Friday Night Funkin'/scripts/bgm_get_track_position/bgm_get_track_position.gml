function bgm_get_track_position(slot){
	bgm_init();
	...
	if audio_is_playing(global.__bgm_id[slot]) {
		return audio_sound_get_track_position(global.__bgm_id[slot]);
	}else{
		trace("Warning! No audio in slot "+string(slot)+" is being played while trying to get the track position in it! Try using bgm_play!");
		return -1;
	}
}
