function bgm_get_volume(slot){
	...
	if audio_is_playing(global.__bgm_id[slot]) {
		return audio_sound_get_gain(global.__bgm_id[slot]);
	}else{
		trace("Warning! No audio in slot "+string(slot)+" is being played while trying to get the volume in it! Try using bgm_play!");
		return -1;
	}
}