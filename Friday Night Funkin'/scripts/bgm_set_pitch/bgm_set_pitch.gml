function bgm_set_pitch(slot,pitch){
	bgm_init();
	...
	if audio_is_playing(global.__bgm_id[slot]) {
		audio_sound_pitch(global.__bgm_id[slot],pitch);
		return 1;
	}else{
		trace("Warning! No audio in slot "+string(slot)+" is being played while trying to set the pitch in it! Try using bgm_play!");
		return -1;
	}
}
