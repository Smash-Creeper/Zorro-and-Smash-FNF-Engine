function bgm_set_track_position(slot,time){
	bgm_init();
	...
	if audio_is_playing(global.__bgm_id[slot]) {
		audio_sound_set_track_position(global.__bgm_id[slot],time);
		return 1;
	}else{
		trace("Warning! No audio in slot "+string(slot)+" is being played while trying to set the track position in it! Try using bgm_play!");
		return -1;
	}
}
