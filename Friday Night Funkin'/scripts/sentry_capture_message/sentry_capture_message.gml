///@arg message a message or exception to capture
///@arg level* the level of the message: SENTRY_CRITICAL, SENTRY_ERROR, SENTRY_WARNING, SENTRY_INFO, SENTRY_DEBUG
///@arg logger* optional logger name
///@arg vars* object or instance to dump variables from
function sentry_capture_message() {
	...
	var parseException= (argument_count> 4)?bool(argument[4]): 0;
	with sentry_handler {
		if is_undefined(endpoint) || is_undefined(dsnPublicKey) {
			show_error("Sentry Error: No endpoint or key, did you run sentry_init with your DSN, and did it succeed?",1);	
		}
		...
		if parseException {
			...
			do { //single do loop that I can break out of
				...
				while string_pos("\r\n",str)!= 0 {
					...
					if at {continue;} //skips the first stack trace line if first stack context already provided
					if string_copy(line,1,4)= " at " {
						at= 1; //pre-providing first stack context
						line= string_delete(line,1,4);
						linePos-= 4;
						bracketPos-= 4;
					}
					var function_= string_copy(line,1,linePos-1);
					...
					pos= string_pos("called from - ",function_);
					if pos!= 0 {
						function_ = string_delete(function_, pos, pos+14);
					}
					if string_pos("gml_Script_",function_)= 1 { //a script
						function_= string_delete(function_,1,11);
						ds_map_add(frame,"module",function_);
					}else if string_pos("gml_Object_",function_)= 1 { //an object
						function_= string_delete(function_,1,11);
						...
						do {
							f1= f2;
							f2= underscore;
							var stub= string_copy(function_,underscore,string_length(function_)-underscore+1);
							var find= string_pos("_",stub);
							underscore+= find;
						}until find= 0 || max_iter--<= 0;
						var module= string_delete(function_,1,f1-1);
						function_= string_copy(function_,1,f1-2);
						ds_map_add(frame,"module",module);
					}
					ds_map_add(frame,"function",function_);
			...
			for (var i= 1; i< array_length_1d(stack); i++;) { //ignore first item to avoid reporting this function
				...
				if linePos {
					var frame= ds_map_create();
					var function_= string_copy(script, 1, linePos-1);
		...
		buffer_delete(buff);
		if ds_map_find_value(options,"backup_before_send") {
